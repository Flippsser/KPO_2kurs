...
namespace Lexer
{
	...
	void checkElInIdTable(std::vector<std::pair<std::string, int>> words, int i, LT::LexTable& lextable, IT::IdTable& idtable, int indIdTab)
	{
		...
		//std::cout << "=== ОТЛАДКА ID ===" << std::endl;
		//std::cout << "Слово: " << words[i].first << std::endl;
		//std::cout << "Размер ID таблицы: " << idtable.size << "/" << idtable.maxsize << std::endl;
		//std::cout << "Размер LEX таблицы: " << lextable.size << "/" << lextable.maxsize << std::endl;

		if (words[i].first.length() > MAX_IDENTIFIER_LENGTH) {
			ERROR_THROW(119);
		}
		...
		try {
			...
			for (int i = 0; !fEnd; i++)
			{
				...
				if (!fEnd)
				{
					...
					else if (in.text[i] == ',' || in.text[i] == ';' || in.text[i] == '{' || in.text[i] == '}' ||
			...
			for (int i = 0; i < size; i++)
			{
				...
				else if (check(words[i].first, typeEquals))
				{
					...
					throw Error::ERROR{ 120, "Не удалось определить лексему", {-1,-1} };
				}
			}
			...
			for (int i = 0; i < idtable.size; i++) {
				...
				cout << ent.id << ", ";
				if (ent.iddatatype == 1) cout << "INT";
				else cout << "STR";
				cout << ", ";
				if (ent.idtype == 1) cout << "V";
				else if (ent.idtype == 2) cout << "F";
				else cout << "P";
				cout << ", " << ent.idxfirstLE << ", ";
				if (ent.iddatatype == IT::INT) cout << ent.value.vint;
				else {
					for (int j = 0; j < ent.value.vstr->len; j++) cout << ent.value.vstr->str[j];
				}
				cout << endl;
			}
		}
